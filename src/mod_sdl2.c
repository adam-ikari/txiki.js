...
static JSValue tjs_sdl_initialize(JSContext *ctx, JSValueConst this_val,
...
static JSValue tjs_sdl_quit(JSContext *ctx, JSValueConst this_val,
                           int argc, JSValueConst *argv) {
  SDL_Quit();
  return JS_UNDEFINED;
}

static JSValue tjs_sdl_poll_events(JSContext *ctx, JSValueConst this_val,
                                 int argc, JSValueConst *argv) {
  SDL_Event event;
...
static JSValue tjs_sdl_create_window(JSContext *ctx, JSValueConst this_val,
                                    int argc, JSValueConst *argv) {
  int x, y, w, h;
  uint32_t flags;
...
static const JSCFunctionListEntry sdl_funcs[] = {
    JS_CFUNC_DEF("init", 0, tjs_sdl_initialize),
